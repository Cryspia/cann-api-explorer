/*
 * Hand-written AscendC high-level math unit: ClampMax (adv_api, simple count mode, no Tiling needed).
 * ClampMax<T>(dst, src, scalar, count): dst[i] = min(src[i], scalar) (clamp values above scalar down to scalar).
 * src filled with 5.0, scalar=3.0 -> dst ~ 3.0.
 */
...
class KernelClampMax {
public:
    __aicore__ inline KernelClampMax() {}
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::ClampMax<DT>(zL, xL, (DT)3.0f, N);            // dst = min(src, 3)
        outZ.EnQue<DT>(zL);
        inX.FreeTensor(xL);
    }
