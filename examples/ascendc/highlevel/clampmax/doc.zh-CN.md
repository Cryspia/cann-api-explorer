...
- 分类：矢量计算 / highlevel（高阶 math，`dst = min(src, scalar)` 逐元素（adv_api，简单 count 模式））
- dtype：float（本例）；头文件另支持 half, float
- 原文：见 CANN 9.1.0 Ascend C API 参考

## 功能
AscendC 高阶 math 接口 `ClampMax`，将所有大于 `scalar` 的元素替换为 `scalar`（即 `dst[i] = min(src[i], scalar)`）。
...
## 最简 example 设计
- src 全填 `5.0`，`scalar = 3.0` → 期望 `dst ≈ 3.0`；host 侧逐元素校验（tol 5e-3）。
