...
## 功能
AscendC adv_api 接口 `LogicalOr`：逐元素逻辑或，`dst = (src0 != 0) || (src1 != 0)`，对 LocalTensor 按元素计算。结果以 `bool`（1 字节，0/1）写出。

## 函数原型（count 模式，摘自 toolkit 头 `logical_or.h`）
```cpp
void LogicalOr(const LocalTensor<T>& dst, const LocalTensor<U>& src0, const LocalTensor<U>& src1, const uint32_t count)
```

## 最简 example 设计
- 输入：src0 = `1.0`，src1 = `0.0` → 期望 `dst = 1`（true）。host 侧把 `bool` 输出按 `uint8_t` 读取并校验 0/1。
