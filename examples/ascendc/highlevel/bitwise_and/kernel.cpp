/*
 * AscendC high-level math operator BitwiseAnd (adv_api, simple count mode, no Tiling needed).
 * Elementwise bitwise AND of two integer tensors: dst[i] = src0[i] & src1[i].
 * Relation: this is the adv_api (lib/math) count-mode entry; the same per-element bitwise AND
 * is also exposed by the lower-level vector intrinsic AscendC::And (see ../../vector/and/).
 * Signature on __NPU_ARCH__==3510:
 *   BitwiseAnd<config, T>(const LocalTensor<T>& dst, const LocalTensor<T>& src0,
 ...
 * so int16_t is used. Example: 0b1100 (12) & 0b1010 (10) = 0b1000 (8).
 */
#include "kernel_operator.h"
#include "lib/math/bitwise_and.h"
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::BitwiseAnd(zL, xL, yL, N);   // dst = src0 & src1
        outZ.EnQue<DT>(zL);
        inX.FreeTensor(xL);
        inY.FreeTensor(yL);
    }
