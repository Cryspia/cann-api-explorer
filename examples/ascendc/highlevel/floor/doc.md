...
## Functionality
AscendC vector compute interface `Floor`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `floor.h`)
```cpp
void Floor(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
```

## Minimal example design
- src filled entirely with `2.7` -> expect `dst ~ 2.0`, host-side element-wise verify (tol 1e-3).
