/*
 * Hand-written high-level unit: ReduceAll (adv_api reduce, last-axis reduction).
 * ReduceAll<T, Pattern::Reduce::AR>(dst, src, sharedTmpBuffer, srcShape, srcInnerPad):
 *   logical "all elements true" reduction along the last (R) axis. For uint8_t input
 *   the result is the min element, so all nonzero -> nonzero, any zero -> 0.
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::ReduceAll<DT, AscendC::Pattern::Reduce::AR>(zL, xL, tmp, srcShape, false);
