/*
 * Host: single-core launch of the ReduceAll kernel. Input is one uint8 vector of
 * length 64 that is all ones. "All true" -> dst[0]=1.
 */
#include <cstdio>
#include <cstdint>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    for (int32_t i = 0; i < N; i++) xH[i] = 1;   // every element true -> "all" is true
    ...
    const int expect = 1;            // all elements true
    int errors = 0;
    ...
    if (errors == 0) printf("REDUCEALL SIMULATION PASSED\n");
    else             printf("REDUCEALL SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
