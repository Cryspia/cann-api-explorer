...
- 关系：tensor-tensor 逻辑算子（见 `../logical_and/`）的 **tensor-scalar** 版本，逐元素逻辑或语义相同，只是 API 入口不同（第二个操作数为标量，广播到所有元素）。
- 原文：见 CANN 9.1.0 Ascend C API 参考（`adv_api/math/logical_ors.h`）

## 功能
AscendC adv_api 接口 `LogicalOrs`：张量与标量逐元素逻辑或，`dst = (src0 != 0) || (scalar != 0)`。结果以 `bool`（1 字节，0/1）写出。当 `scalar != 0` 时结果恒为真；当 `scalar == 0` 时退化为 `src0` 的真假。

## 函数原型（count 模式，摘自 toolkit 头 `logical_ors.h`，__NPU_ARCH__==3510）
```cpp
...
## 最简 example 设计
- 输入：src0 = `0.0`（假），标量 = `1`（真）→ 期望 `dst = 1`（true）。host 侧把 `bool` 输出按 `uint8_t` 读取并校验 0/1。
