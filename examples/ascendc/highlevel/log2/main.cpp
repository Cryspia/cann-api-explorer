 ...
 * The input is filled with the constant 8.0; after the run the host-side verify expects ~ 3.0. dtype=float.
 * log2(8) = 3.
 */
#include <cstdlib>
#include <cstdio>
...
int32_t main()
{
    ...
    if(!golden) for (int32_t i = 0; i < totalLen; i++) xHost[i] = (float)(8.0);
    ...
    if (errors == 0) printf("LOG2 SIMULATION PASSED\n");
    else             printf("LOG2 SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
