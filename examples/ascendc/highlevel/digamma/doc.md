...
## Functionality
AscendC vector compute interface `Digamma`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `digamma.h`)
```cpp
void Digamma(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
```

## Minimal example design
- src filled entirely with `1.0` -> expect `dst ~ -0.5772157`, host-side element-wise verify (tol 5e-3).
