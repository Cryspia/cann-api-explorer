...
## Functionality
AscendC vector compute interface `Lgamma`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `lgamma.h`)
```cpp
void Lgamma(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
```

## Minimal example design
- src filled entirely with `1.0` -> expect `dst ~ 0.0`, host-side element-wise verify (tol 5e-3).
