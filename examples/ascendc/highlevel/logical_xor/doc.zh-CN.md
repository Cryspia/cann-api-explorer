...
- dtype：dst 为 `bool`；src 支持整型/浮点族。本例：src `float` → dst `bool`。
- 原文：见 CANN 9.1.0 Ascend C API 参考

## 功能
AscendC adv_api 接口 `LogicalXor`：逐元素逻辑异或，`dst = (src0 != 0) ^ (src1 != 0)`，对 LocalTensor 按元素计算。结果以 `bool`（1 字节，0/1）写出。

## 函数原型（count 模式，摘自 toolkit 头 `logical_xor.h`）
```cpp
void LogicalXor(const LocalTensor<T>& dst, const LocalTensor<U>& src0, const LocalTensor<U>& src1, const uint32_t count)
```

## 最简 example 设计
- 输入：src0 = `1.0`，src1 = `0.0` → 期望 `dst = 1`（true）；注意 `1 ^ 1 = 0`。host 侧把 `bool` 输出按 `uint8_t` 读取并校验 0/1。
