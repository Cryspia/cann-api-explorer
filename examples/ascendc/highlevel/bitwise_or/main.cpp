/*
 * Host launcher (two integer tensor inputs): launches the k_custom kernel on the simulated Ascend device.
 * dtype=int16_t (exact bit-for-bit match). x=12 (0b1100), y=10 (0b1010), verify x | y == 14 (0b1110).
 */
#include <cstdio>
#include <cstdint>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    for (int32_t i = 0; i < totalLen; i++) { xHost[i] = (DT)(12); yHost[i] = (DT)(10); }
    ...
    const DT expect = (DT)(14);  // 12 | 10 = 14
    int errors = 0;
    ...
    if (errors == 0) printf("BITWISE_OR SIMULATION PASSED\n");
    else             printf("BITWISE_OR SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
