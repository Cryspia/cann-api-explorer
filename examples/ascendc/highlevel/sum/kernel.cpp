/*
 * Hand-written high-level unit: Sum (adv_api reduce, last-axis reduction with SumParams + tmp).
 * Sum<T>(dst, src, sharedTmpBuffer, SumParams{outter, inner, n}):
 *   reduces each of `outter` rows (row stride `inner`, valid length `n`) to a single sum.
 * Layout [outter=1, n=64], inner=64 (inner*sizeof(float)=256, multiple of 32).
 * src all 1.0 -> sum over 64 elements = 64.0, written to dst[0].
 */
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::SumParams params{OUTTER, INNER, N};
        AscendC::Sum<DT>(zL, xL, tmp, params);
