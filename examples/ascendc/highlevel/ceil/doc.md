...
## Functionality
AscendC vector compute interface `Ceil`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `ceil.h`)
```cpp
void Ceil(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
```

## Minimal example design
- src filled entirely with `2.3` -> expect `dst ~ 3.0`, host-side element-wise verify (tol 1e-3).
