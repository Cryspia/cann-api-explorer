/*
 * Host launcher for AscendDequant: source int32 filled with 4, dequantized by k_custom
 * with deqScale=0.5 -> dst(float) = 4 * 0.5 = 2.0. Verify == 2.0 (tol 1e-4).
 */
#include <cstdio>
#include <cstdint>
#include <cmath>
...
int32_t main()
{
    ...
    for (int32_t i = 0; i < totalLen; i++) xHost[i] = (ST)(4);
    ...
    const DT expect = (DT)(2.0);
    int errors = 0;
    for (int32_t i = 0; i < totalLen; i++) {
        if (std::fabs((double)(zHost[i] - expect)) > 1e-4) {
            if (errors < 5) printf("[CHECK] idx %d = %f (expect %f)\n",
                                   i, (double)zHost[i], (double)expect);
            errors++;
        }
    }
    printf("z[0]=%f z[last]=%f total=%d errors=%d\n",
           (double)zHost[0], (double)zHost[totalLen - 1], totalLen, errors);

    // Emit the PASS/FAIL marker BEFORE ACL teardown.
    if (errors == 0) printf("ASCENDDEQUANT SIMULATION PASSED\n");
    else             printf("ASCENDDEQUANT SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
