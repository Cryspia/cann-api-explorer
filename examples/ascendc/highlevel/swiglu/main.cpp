/*
 * Host: single-core launch of the SwiGLU kernel.
 * src0 = 4.0, src1 = 3.0, beta = 0 -> dst = src0 * (src1 / (1 + exp(0))) = 4 * 3 / 2 = 6.0.
 */
#include <cstdio>
#include <cstdint>
#include <cmath>
...
int32_t main()
{
    ...
    for (int32_t i = 0; i < N; i++) { x0H[i] = 4.0f; x1H[i] = 3.0f; }
    ...
    if (errors == 0) printf("SWIGLU SIMULATION PASSED\n");
    else             printf("SWIGLU SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
