/* Host: GeGLU, src0=1.0, src1=0.0 -> GeLU(0)=0 -> expect dst = 1 * 0 = 0.0. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    const float expect = 0.0f;   // GeGLU(1, 0) = 1 * GeLU(0) = 0
    int errors = 0;
    ...
    if (errors == 0) printf("GEGLU SIMULATION PASSED\n");
    else             printf("GEGLU SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
