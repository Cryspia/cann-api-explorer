...
## Functionality
AscendC vector compute interface `Hypot`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `hypot.h`)
```cpp
...
- src0=`3.0`, src1=`4.0` -> expect `dst ~ 5.0`, host-side element-wise verify (tol 5e-3).
