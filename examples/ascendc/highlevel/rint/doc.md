...
## Functionality
AscendC vector compute interface `Rint`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `rint.h`)
```cpp
void Rint(const LocalTensor<T>& dst, const LocalTensor<T>& src, const uint32_t count)
```
