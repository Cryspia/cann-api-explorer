...
## 功能
AscendC 数学接口 `Log10`，对 LocalTensor 按元素计算以 10 为底的对数。
这是独立函数，与 `Log`（自然对数）和 `Log2` 不同。

## 函数原型（count 模式，摘自 toolkit 头 `log.h`）
```cpp
void Log10(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, uint32_t calCount)
```
注：在 __NPU_ARCH__==3510 上，`Log10`（count 模式）无需临时 buffer。

## 最简 example 设计
- src 全填 `1000.0` → 期望 `dst ≈ 3.0`（log10(1000) = 3），host 侧逐元素校验（tol 5e-3）。
