 ...
 * The input is filled with the constant 1000.0; after the run the host-side verify expects ~ 3.0. dtype=float.
 * log10(1000) = 3.
 */
#include <cstdlib>
#include <cstdio>
...
int32_t main()
{
    ...
    if(!golden) for (int32_t i = 0; i < totalLen; i++) xHost[i] = (float)(1000.0);
    ...
    const float tol = (float)(5e-3);
    int errors = 0;
    for (int32_t i = 0; i < totalLen; i++) {
        float v = zHost[i];
        if (fabsf(v - expect) > tol) {
    ...
    if (errors == 0) printf("LOG10 SIMULATION PASSED\n");
    else             printf("LOG10 SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
