...
## Functionality
AscendC vector compute interface `Gelu`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `gelu.h`)
```cpp
void Gelu(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal, const uint32_t dataSize)
```

## Minimal example design
- src filled entirely with `1.0` -> expect `dst ~ 0.8413447`, host-side element-wise verify (tol 1e-2).
