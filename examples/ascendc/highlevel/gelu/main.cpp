 ...
 * The input is filled with the constant 1.0; after the run the host-side verify expects ~ 0.8413447. dtype=float.
 */
#include <cstdio>
#include <cstdint>
#include <cmath>
...
int32_t main()
{
    ...
    const float expect = (float)(0.8413447);
    int errors = 0;
    for (int32_t i = 0; !golden && i < totalLen; i++) {
        float v = zHost[i];
        if (fabsf(v - expect) > 1e-2f) {
    ...
    if (errors == 0) printf("GELU SIMULATION PASSED\n");
    else             printf("GELU SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
