 ...
 * The input is filled with the constant 1.0; after the run the host-side verify expects ~ 0.7310586. dtype=float.
 */
#include <cstdio>
#include <cstdint>
#include <cmath>
...
int32_t main()
{
    ...
    const float expect = (float)(0.7310586);
    int errors = 0;
    for (int32_t i = 0; !golden && i < totalLen; i++) {
        float v = zHost[i];
    ...
    if (errors == 0) printf("SIGMOID SIMULATION PASSED\n");
    else             printf("SIGMOID SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
