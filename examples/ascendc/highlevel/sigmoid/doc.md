...
## Functionality
AscendC vector compute interface `Sigmoid`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `sigmoid.h`)
```cpp
void Sigmoid(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
```
