...
## Functionality
AscendC vector compute interface `Frac`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `frac.h`)
```cpp
void Frac(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
```

## Minimal example design
- src filled entirely with `2.75` -> expect `dst ~ 0.75`, host-side element-wise verify (tol 5e-3).
