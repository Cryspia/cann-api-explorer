...
## 功能
AscendC adv_api 接口 `IsInf`：逐元素无穷判断，`dst = isinf(src)`，对 LocalTensor 按元素计算。结果以 `bool`（1 字节，0/1）写出。

## 函数原型（count 模式，摘自 toolkit 头 `is_inf.h`）
```cpp
void IsInf(const LocalTensor<T>& dst, const LocalTensor<U>& src, const uint32_t count)
```

## 最简 example 设计
- 输入：src = `1.0` → 期望 `dst = 0`（false）。host 侧把 `bool` 输出按 `uint8_t` 读取并校验 0/1。
