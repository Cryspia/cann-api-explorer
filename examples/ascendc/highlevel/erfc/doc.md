...
## Functionality
AscendC vector compute interface `Erfc`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `erfc.h`)
```cpp
void Erfc(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
```

## Minimal example design
- src filled entirely with `0.0` -> expect `dst ~ 1.0`, host-side element-wise verify (tol 5e-3).
