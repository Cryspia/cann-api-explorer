/* Host: UnPad, src=[1..16] (srcWidth=16, rightPad=4) -> dst[0..11]=[1..12] (drop last 4 columns). */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    const int32_t DST_W = 12;   // valid width = srcWidth - rightPad
    const size_t bytes = (size_t)SRC_W * sizeof(float);
    ...
    for (int i = 0; i < SRC_W; i++) { xH[i] = (float)(i + 1); zH[i] = -1.0f; }
    ...
    for (int i = 0; i < DST_W; i++) {
        float ev = (float)(i + 1);
        if (zH[i] != ev) { if (errors < 5) printf("[CHECK] z[%d]=%g (expect %g)\n", i, zH[i], ev); errors++; }
    }
    printf("z[0..3]=[%g,%g,%g,%g] z[11]=%g (expect 1,2,3,4 .. 12) errors=%d\n",
           zH[0], zH[1], zH[2], zH[3], zH[11], errors);

    if (errors == 0) printf("UNPAD SIMULATION PASSED\n");
    else             printf("UNPAD SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
