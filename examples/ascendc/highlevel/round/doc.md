...
## Functionality
AscendC vector compute interface `Round`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `round.h`)
```cpp
void Round(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
```

## Minimal example design
- src filled entirely with `2.4` -> expect `dst ~ 2.0`, host-side element-wise verify (tol 1e-3).
