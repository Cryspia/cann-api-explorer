/*
 * Hand-written vector unit: Ors (per-element bitwise OR of a tensor with a scalar, count mode).
 ...
 * so this unit uses int16_t). src0[i]=6, scalar=3 -> dst[i]=7.
 */
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::Ors(zL, xL, SCALAR, (int32_t)N);
        outZ.EnQue<DT>(zL);
        inX.FreeTensor(xL);
    }
