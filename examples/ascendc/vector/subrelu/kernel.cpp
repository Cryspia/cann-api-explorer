/*
 * Hand-written vector unit: SubRelu (fused add + ReLU, count mode).
 * SubRelu<T>(dst, src0, src1, count): dst[i] = max(src0[i] - src1[i], 0).
 * Test src0=5, src1=2 -> dst = max(3, 0) = 3.
 */
...
class KernelSubRelu {
public:
    __aicore__ inline KernelSubRelu() {}
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::SubRelu<DT>(zL, x0L, x1L, N);                   // dst = max(src0 - src1, 0)
        outZ.EnQue<DT>(zL);
        inX0.FreeTensor(x0L);
        inX1.FreeTensor(x1L);
    }
