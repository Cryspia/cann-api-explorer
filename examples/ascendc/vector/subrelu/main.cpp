/* Host: SubRelu, src0=5, src1=2 -> expect dst = max(5-2,0) = 3. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    for (int i = 0; i < N; i++) { x0H[i] = 5.0f; x1H[i] = 2.0f; }
    ...
    printf("z[0..3]=[%g,%g,%g,%g] (expect 3) errors=%d\n", zH[0], zH[1], zH[2], zH[3], errors);
    ...
    if (errors == 0) printf("SUBRELU SIMULATION PASSED\n");
    else             printf("SUBRELU SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
