...
## Functionality
AscendC vector compute interface `Div`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `kernel_operator_vec_binary_intf.h`)
```cpp
void Div(const LocalTensor<T>& dst, const LocalTensor<T>& src0, const LocalTensor<T>& src1, const int32_t& count)
```

## Minimal example design
- src0=`6.0`, src1=`2.0` -> expect `dst ~ 3.0`, host-side element-wise verify (tol 1e-3).
