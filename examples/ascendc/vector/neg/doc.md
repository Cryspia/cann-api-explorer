...
## Functionality
AscendC vector compute interface `Neg`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `kernel_operator_vec_unary_intf.h`)
```cpp
void Neg(const LocalTensor<T> &dst, const LocalTensor<T> &src, const uint32_t count)
```

## Minimal example design
- src filled entirely with `-3.0` -> expect `dst ~ 3.0`, host-side element-wise verify (tol 1e-3).
