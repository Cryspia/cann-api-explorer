...
## Functionality
AscendC vector compute interface `Reciprocal`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `kernel_operator_vec_unary_intf.h`)
```cpp
void Reciprocal(const LocalTensor<T>& dst, const LocalTensor<T>& src, const int32_t& count)
```

## Minimal example design
- src filled entirely with `4.0` -> expect `dst ~ 0.25`, host-side element-wise verify (tol 1e-3).
