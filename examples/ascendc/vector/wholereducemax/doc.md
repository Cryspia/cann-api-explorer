...
## Functionality
`WholeReduceMax` finds the max of each repeat. With `ORDER_ONLY_VALUE` only the value is emitted (1 element/repeat).
...
- src[i] = i%64 -> each repeat is 0..63, max = 63.0 (8 outputs).
