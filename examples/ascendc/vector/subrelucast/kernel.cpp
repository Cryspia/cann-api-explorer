/*
 * Hand-written vector unit: SubReluCast (fused sub + ReLU + type conversion, count mode).
 * SubReluCast<T,U>(dst, src0, src1, count): dst = cast_to_T( relu(src0 - src1) ).
 * Supported dtype combination on this SoC (Level 2): src(U)=float, dst(T)=half.
 * No deqScale needed (unlike AddDeqRelu).
 * src0[i] alternates -1/5, src1=2 -> relu(-3)=0 and relu(3)=3, cast to half.
 */
...
class KernelSubReluCast {
public:
    __aicore__ inline KernelSubReluCast() {}
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::SubReluCast<DT, ST>(zL, x0L, x1L, N);          // dst = cast(relu(src0 - src1))
        outZ.EnQue<DT>(zL);
        inX0.FreeTensor(x0L);
        inX1.FreeTensor(x1L);
    }
