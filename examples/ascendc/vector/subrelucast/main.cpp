/* Host: SubReluCast. src0[i]= (i even ? -1 : 5), src1=2 (float) -> dst = half(relu(src0-2)).
 * even: relu(-1-2)=relu(-3)=0 ; odd: relu(5-2)=relu(3)=3. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    for (int i = 0; i < N; i++) { x0H[i] = (i % 2 == 0) ? -1.0f : 5.0f; x1H[i] = 2.0f; }
    ...
    if (errors == 0) printf("SUBRELUCAST SIMULATION PASSED\n");
    else             printf("SUBRELUCAST SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
