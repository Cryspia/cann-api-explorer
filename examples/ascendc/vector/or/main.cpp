/*
 * Auto-generated host launcher (two inputs, integer bitwise ops): x=6 y=3, verify == 7.
 * dtype=int16_t (exact bit-for-bit match).
 */
#include <cstdio>
#include <cstdint>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    for (int32_t i = 0; i < totalLen; i++) { xHost[i] = (DT)(6); yHost[i] = (DT)(3); }
    ...
    if (errors == 0) printf("OR SIMULATION PASSED\n");
    else             printf("OR SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
