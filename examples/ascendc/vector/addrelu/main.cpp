/* Host: AddRelu, src0=1, src1=2 -> expect dst = max(1+2,0) = 3. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    for (int i = 0; i < N; i++) { x0H[i] = 1.0f; x1H[i] = 2.0f; }
    ...
    if (errors == 0) printf("ADDRELU SIMULATION PASSED\n");
    else             printf("ADDRELU SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
