/* Host: MulAddRelu, preset dst=1, src0=2, src1=3 -> expect dst = relu(src0*dst+src1) = relu(2*1+3) = 5. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    for (int i = 0; i < N; i++) { x0H[i] = 2.0f; x1H[i] = 3.0f; }
    ...
    int errors = 0;
    const float ev = 5.0f;
    for (int i = 0; i < N; i++) { if (fabsf(zH[i] - ev) > 1e-3f) { if (errors < 5) printf("[CHECK] z[%d]=%g (expect %g)\n", i, zH[i], ev); errors++; } }
    printf("z[0..3]=[%g,%g,%g,%g] (expect 5) errors=%d\n", zH[0], zH[1], zH[2], zH[3], errors);
    ...
    if (errors == 0) printf("MULADDRELU SIMULATION PASSED\n");
    else             printf("MULADDRELU SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
