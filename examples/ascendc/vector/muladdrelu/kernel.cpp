/*
 * Hand-written vector unit: MulAddRelu (count mode).
 * MulAddRelu<T>(dst, src0, src1, count): dst[i] = max(src0[i] * dst[i] + src1[i], 0) (accumulates into dst, then ReLU).
 * dst must be preset first (Duplicate). Preset dst=1, src0=2, src1=3 -> relu(2*1+3) = 5.
 */
...
class KernelMulAddRelu {
public:
    __aicore__ inline KernelMulAddRelu() {}
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::MulAddRelu<DT>(zL, x0L, x1L, N);               // dst = max(src0 * dst + src1, 0)
        outZ.EnQue<DT>(zL);
        inX0.FreeTensor(x0L);
        inX1.FreeTensor(x1L);
    }
