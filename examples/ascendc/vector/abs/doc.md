...
## Functionality
AscendC vector compute interface `Abs`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `kernel_operator_vec_unary_intf.h`)
```cpp
void Abs(const LocalTensor<T>& dst, const LocalTensor<T>& src, const int32_t& count)
```
