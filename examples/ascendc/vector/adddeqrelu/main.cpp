/* Host: AddDeqRelu. deqScale=2.0. src0/src1 (int32) alternate:
 * even: (1+2)*2 = 6 ; odd: (-5+1)*2 = -8 -> relu -> 0. dst is half. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    int32_t *x0H = nullptr, *x1H = nullptr; uint16_t *zH = nullptr;
    ...
    for (int i = 0; i < N; i++) {
        if (i % 2 == 0) { x0H[i] = 1;  x1H[i] = 2; }
        else            { x0H[i] = -5; x1H[i] = 1; }
    }
    ...
    for (int i = 0; i < N; i++) {
        ...
        float ev  = (i % 2 == 0) ? 6.0f : 0.0f;
        if (fabsf(got - ev) > 1e-2f) { if (errors < 5) printf("[CHECK] z[%d]=%g (expect %g)\n", i, got, ev); errors++; }
    }
    printf("z[0]=%g z[1]=%g (expect 6, 0) errors=%d\n", half_to_float(zH[0]), half_to_float(zH[1]), errors);

    if (errors == 0) printf("ADDDEQRELU SIMULATION PASSED\n");
    else             printf("ADDDEQRELU SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
