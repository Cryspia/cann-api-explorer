/* Host: Ands, src0[i]=6, scalar=3 (int16) -> expect dst[i] = 6 & 3 = 2. */
#include <cstdio>
#include <cstdint>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    printf("z[0..3]=[%d,%d,%d,%d] errors=%d\n", (int)zH[0], (int)zH[1], (int)zH[2], (int)zH[3], errors);
    ...
    if (errors == 0) printf("ANDS SIMULATION PASSED\n");
    else             printf("ANDS SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
