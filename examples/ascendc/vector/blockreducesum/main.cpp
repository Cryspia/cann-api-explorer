/* Host launcher for BlockReduceSum: Sum the 8 floats in each 32B block; one 256B repeat (64 floats) yields 8 block sums. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    const int32_t DSTPAD = 64;   // padded (32B-aligned) output length copied from device
    const int32_t NOUT = 64;       // number of meaningful output elements to verify
    ...
    for (int i = 0; i < SRC; i++) xH[i] = (float)(1.0);
    ...
    int errors = 0;
    for (int i = 0; i < 64; i++) { float ev = (float)(8.0); if (fabsf(zH[i]-ev) > 1e-3f) { if (errors<5) printf("[CHECK] z[%d]=%g (expect %g)\n", i, zH[i], ev); errors++; } }

    printf("z[0..2]=[%g,%g,%g] errors=%d\n", zH[0], NOUT>1?zH[1]:0.0f, NOUT>2?zH[2]:0.0f, errors);

    // Emit the PASS/FAIL marker BEFORE ACL teardown.
    if (errors == 0) printf("BLOCKREDUCESUM SIMULATION PASSED\n");
    else             printf("BLOCKREDUCESUM SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
