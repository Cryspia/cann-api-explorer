/*
 * Hand-written vector unit: ShiftLeft (per-element logical left shift, two input tensors, count mode).
 * ShiftLeft<T,U>(dst, src0, src1, count): dst[i] = src0[i] << src1[i].
 * Supported dtype combinations on this SoC include T=int32, U=int32 (also int64/uint64, uint32).
 * src0[i]=1, src1[i]=3 -> dst[i]=8.
 */
...
class KernelShiftLeft {
public:
    __aicore__ inline KernelShiftLeft() {}
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::ShiftLeft<T, U>(zL, a, b, N);
        outZ.EnQue<T>(zL);
