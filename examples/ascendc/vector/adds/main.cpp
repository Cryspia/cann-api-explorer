 ...
 * The input is filled with the constant 1.0; after the run the host-side verify expects ~ 3.0. dtype=float.
 */
#include <cstdlib>
#include <cstdio>
#include <cstdio>
...
int32_t main()
{
    ...
    if(!golden) for (int i = 0; i < totalLen; i++) xHost[i] = (float)(1.0);
    ...
    if (errors == 0) printf("ADDS SIMULATION PASSED\n");
    else             printf("ADDS SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
