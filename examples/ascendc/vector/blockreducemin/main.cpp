/* Host launcher for BlockReduceMin: Min of the 8 floats in each 32B block. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    int errors = 0;
    for (int i = 0; i < 64; i++) { float ev = (float)((i % 8) * 8); if (fabsf(zH[i]-ev) > 1e-3f) { if (errors<5) printf("[CHECK] z[%d]=%g (expect %g)\n", i, zH[i], ev); errors++; } }

    printf("z[0..2]=[%g,%g,%g] errors=%d\n", zH[0], NOUT>1?zH[1]:0.0f, NOUT>2?zH[2]:0.0f, errors);

    // Emit the PASS/FAIL marker BEFORE ACL teardown.
    if (errors == 0) printf("BLOCKREDUCEMIN SIMULATION PASSED\n");
    else             printf("BLOCKREDUCEMIN SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
