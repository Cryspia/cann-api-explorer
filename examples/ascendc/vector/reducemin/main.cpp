 ...
 * host-side verify dst[0] ~ 1.0. dtype=float.
 */
#include <cstdlib>
#include <cstdio>
#include <cstdio>
...
int32_t main()
{
    ...
    bool golden=false;
    { const char*gx=getenv("GOLDEN_IN_X"); if(gx){ FILE*fx=fopen(gx,"rb"); if(fx&&fread(xHost,sizeof(float),elem,fx)==(size_t)elem) golden=true; if(fx)fclose(fx);} }
    if(!golden) for (int32_t i = 0; i < elem; i++) xHost[i] = (float)(1.0);
    ...
    if(golden){ const char*go=getenv("GOLDEN_OUT"); if(go){ FILE*fo=fopen(go,"wb"); if(fo){ fwrite(zHost,sizeof(float),1,fo); fclose(fo); printf("[GOLDEN] dumped 1 float\n");} } }
    ...
    if (errors == 0) printf("REDUCEMIN SIMULATION PASSED\n");
    else             printf("REDUCEMIN SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
