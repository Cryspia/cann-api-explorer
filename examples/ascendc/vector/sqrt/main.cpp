 ...
 * The input is filled with the constant 4.0; after the run the host-side verify expects ~ 2.0. dtype=float.
 */
#include <cstdio>
#include <cstdint>
#include <cmath>
...
int32_t main()
{
    ...
    for (int32_t i = 0; i < totalLen; i++) xHost[i] = (float)(4.0);
    // golden bridge: if GOLDEN_IN_X is set, read input from an fp32 raw file to override the constant (shares the same random data with an external cross-check party)
    bool golden = false;
    { const char *gx = getenv("GOLDEN_IN_X");
      if (gx) { FILE *fx = fopen(gx, "rb");
        if (fx && fread(xHost, sizeof(float), totalLen, fx) == (size_t)totalLen) golden = true;
        if (fx) fclose(fx); } }
    ...
    int errors = 0;
    for (int32_t i = 0; !golden && i < totalLen; i++) {   // in golden mode skip the constant self-check (input is external random data)
        float v = zHost[i];
    ...
    if (errors == 0) printf("SQRT SIMULATION PASSED\n");
    else             printf("SQRT SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
