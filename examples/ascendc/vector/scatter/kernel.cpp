/*
 * Hand-written vector unit: Scatter (scatter to dst by offset, count mode).
 * Scatter(dst, src, dstOffset, dstBaseAddr, count): dst[(dstBaseAddr+dstOffset[i])/sizeof(T)] = src[i].
 * dstOffset is a byte offset. In this example src[i]=i, dstOffset[i]=(N-1-i)*4 -> dst[N-1-i]=i -> dst[j]=N-1-j (reverse-order scatter).
 */
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::Scatter<DT>(zL, xL, oL, (uint32_t)0, N);
        outZ.EnQue<DT>(zL);
