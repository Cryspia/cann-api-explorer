/* Host: Scatter, src[i]=i, dstOffset[i]=(N-1-i)*4 bytes -> expect dst[i]=N-1-i (reverse-order scatter). */
#include <cstdio>
#include <cstdint>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    if (errors == 0) printf("SCATTER SIMULATION PASSED\n");
    else             printf("SCATTER SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
