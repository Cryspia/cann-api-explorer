/* Host: MulAddDst, preset dst=1, src0=2, src1=3 -> expect dst = src0*src1+dst = 2*3+1 = 7. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    printf("z[0..3]=[%g,%g,%g,%g] (expect 7) errors=%d\n", zH[0], zH[1], zH[2], zH[3], errors);
    ...
    if (errors == 0) printf("MULADDDST SIMULATION PASSED\n");
    else             printf("MULADDDST SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
