/* Host launcher for PairReduceSum: Sum adjacent pairs; output count is halved. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    const int32_t SRC = 64;
    const int32_t DSTPAD = 32;   // padded (32B-aligned) output length copied from device
    const int32_t NOUT = 32;       // number of meaningful output elements to verify
    ...
    int errors = 0;
    for (int i = 0; i < 32; i++) { float ev = (float)(2.0); if (fabsf(zH[i]-ev) > 1e-3f) { if (errors<5) printf("[CHECK] z[%d]=%g (expect %g)\n", i, zH[i], ev); errors++; } }

    printf("z[0..2]=[%g,%g,%g] errors=%d\n", zH[0], NOUT>1?zH[1]:0.0f, NOUT>2?zH[2]:0.0f, errors);

    // Emit the PASS/FAIL marker BEFORE ACL teardown.
    if (errors == 0) printf("PAIRREDUCESUM SIMULATION PASSED\n");
    else             printf("PAIRREDUCESUM SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
