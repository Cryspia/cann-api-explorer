/*
 * Hand-written vector unit: AbsSub (absolute difference, count mode).
 * AbsSub<T>(dst, src0, src1, count): dst[i] = |src0[i] - src1[i]|.
 * Test src0=2, src1=5 -> dst = |2-5| = 3.
 */
...
class KernelAbsSub {
public:
    __aicore__ inline KernelAbsSub() {}
...
private:
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::AbsSub<DT>(zL, x0L, x1L, N);                   // dst = |src0 - src1|
        outZ.EnQue<DT>(zL);
        inX0.FreeTensor(x0L);
        inX1.FreeTensor(x1L);
    }
