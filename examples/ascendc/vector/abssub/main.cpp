/* Host: AbsSub, src0=2, src1=5 -> expect dst = |2-5| = 3. */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    for (int i = 0; i < N; i++) { x0H[i] = 2.0f; x1H[i] = 5.0f; }
    ...
    if (errors == 0) printf("ABSSUB SIMULATION PASSED\n");
    else             printf("ABSSUB SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
