/* Host launcher for WholeReduceMin: Min of all 64 floats in each repeat (value-only order). */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    int errors = 0;
    for (int i = 0; i < 8; i++) { float ev = (float)(0.0); if (fabsf(zH[i]-ev) > 1e-3f) { if (errors<5) printf("[CHECK] z[%d]=%g (expect %g)\n", i, zH[i], ev); errors++; } }

    printf("z[0..2]=[%g,%g,%g] errors=%d\n", zH[0], NOUT>1?zH[1]:0.0f, NOUT>2?zH[2]:0.0f, errors);

    // Emit the PASS/FAIL marker BEFORE ACL teardown.
    if (errors == 0) printf("WHOLEREDUCEMIN SIMULATION PASSED\n");
    else             printf("WHOLEREDUCEMIN SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
