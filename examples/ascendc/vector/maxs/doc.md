...
## Functionality
AscendC vector compute interface `Maxs`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `kernel_operator_vec_binary_scalar_intf.h`)
```cpp
void Maxs(const U& dst, const S& src0, const V& src1, const int32_t& count)
```

## Minimal example design
- src filled entirely with `1.0`, scalar `2.0` -> expect `dst ~ 2.0`, host-side element-wise verify (tol 1e-3).
