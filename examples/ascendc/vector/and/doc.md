...
## Functionality
AscendC vector compute interface `And`, computes element-wise over a LocalTensor.

## Function prototype (count mode, taken from toolkit header `kernel_operator_vec_binary_intf.h`)
```cpp
void And(const LocalTensor<T>& dst, const LocalTensor<T>& src0, const LocalTensor<T>& src1, const int32_t& count)
```

## Minimal example design
- src0=`6`, src1=`3` -> expect `dst == 2`, host-side element-wise verify (exact match).
