# Ascend C · AddReluCast (vector, fused conv)

> 中文版: [doc.zh-CN.md](doc.zh-CN.md)

- Category: vector / fused add + ReLU + cast
- Covered API: `AddReluCast`
- include: `kernel_operator.h`
- Source: CANN 9.1.0 Ascend C API Reference / AddReluCast

## Functionality
`AddReluCast` fuses element-wise add, ReLU, and a type conversion:
...
- `N = 256`. `src0[i]` alternates `-3 / 2`, `src1 = 1` (float).
- even `i`: `relu(-3 + 1) = relu(-2) = 0`; odd `i`: `relu(2 + 1) = relu(3) = 3`, cast to half.
