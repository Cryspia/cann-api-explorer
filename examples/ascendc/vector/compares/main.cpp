/* Host: Compares(GT 31.5)+Select, src[i]=i -> expect z[i]=(i>=32)?1:0 (vector vs scalar). */
#include <cstdio>
#include <cstdint>
#include <cmath>
#include "acl/acl.h"
...
int32_t main()
{
    ...
    if (errors == 0) printf("COMPARES SIMULATION PASSED\n");
    else             printf("COMPARES SIMULATION FAILED (%d errors)\n", errors);
    fflush(stdout);
