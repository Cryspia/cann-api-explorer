...
两个 CAModel 指标（指令执行数、执行耗时 ns）是器件级的，本应与宿主机无关。在 83 个共有数值单元上实测：

| 指标 | 平均 |Δ| | 中位 |Δ| | 最大 |Δ| |
|---|---|---|---|
| **指令数** | **0.034 %** | 0.009 % | 1.76 %（SyncAll） |
| **执行耗时 (ns)** | 2.22 % | 0.54 % | 18.2 %（小算子，固定开销主导） |

- **指令数基本完全一致**：83 个中 41 个逐位相等；其余仅相差 **+3 … +43** 条指令（基数 2 万~16 万，≤0.03 %）。纯计算/Cube/归一化类核函数（Matmul、各 Norm、SoftMax、Sort、TopK、ScalarCast、数据重排）**完全一致**。
...
| | 数值 |
|---|---|
| 84 个单元总墙钟（串行） | **约 112 分钟**（6709 s） |
| 单元区间 | **43.8 s – 167.5 s** |
| 单元中位 / 平均 | 87 s / 80 s |
| 固定下限（最小 204 指令单元） | **约 44 s** |

- 每个单元有 **约 44 s 的固定启动下限**（CAModel 初始化 + SoC 拉起 + 退出），与核函数大小无关——即便 204 条指令的 `Duplicate` 也要约 46 s。
- 下限之上，墙钟**随指令数增长**：`Lgamma`（16 万指令）最慢 168 s；多核同步单元（`IBSetWait` 115 s、`SyncAll` 106 s）次之，因轮询放大了被仿真的工作量。
