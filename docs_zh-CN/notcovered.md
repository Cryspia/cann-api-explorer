...
| **aolapi / AOL**（算子加速库） | 预编译好的高层算子库（类比 cuDNN） | 是 host 侧加载/调用预编译算子二进制，不暴露可仿真的核函数源码 | 真实设备 + 算子二进制包；调用即黑盒，无指令级 trace |
...
- **A 线（已完成，82/82）**：Ascend C 核函数 API —— CAModel 指令级仿真的唯一对象，本项目核心。
- **B 线（已完成）**：Runtime/AscendCL host API —— 发射核函数的脚手架，见 [`runtime/host_api.md`](runtime/host_api.md)，不产指令报告但其正确性隐含在 A 线 82 个 PASSED 单元中。
