...
- **Track A (simulatable Ascend C kernel API) = core ✅ done (118/118)**: 9 arity families + 4 tiling techniques (device TilingFunc / few-field hand-fill / host-tiling framework / tiling-free) + Cube (Matmul) + multi-core sync (SyncAll/IBSet/IBWait) + data-rearrange/atomic/scalar. **Computation-class operators are now near-exhaustively covered** — beyond the mainstream set: fused binary (AddRelu/SubRelu/FusedMulAdd/MulAddDst/MulAddRelu/AbsSub/ExpSub/Mull), shifts (ShiftLeft/ShiftRight), scalar variants (Ands/Ors/LeakyRelu/MulsCast), Prelu, inverse-hyperbolic (Acosh/Asinh/Atanh), Digamma, ClampMax/Min, Fma, SinCos, GeGLU, Xor, logical (LogicalAnd/Or/Xor/Not — bool out), predicate (IsNan/IsInf/IsFinite — bool out), Where, CumSum. Remaining uncovered (quantized AddDeqRelu, Philox RNG, arch-limited Conv3D/Conv2D) are itemized in [`docs/INDEX.md`](docs/INDEX.md).
